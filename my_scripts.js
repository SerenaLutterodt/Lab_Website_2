...
*/
function vewStudentStats(id, toggle)
{
	if(toggle == 1)
	{
...
/*
	Home Page:
		changeColor(color) method
			parameter:
				color- A css color
				alert("Function called with paramenter:" + color)
...
{
	alert("Function called with parameter: " + color)
	document.body.style.background = color;
}
...
function loadStatsPage()
{
	...
	var score;
	var winCounter = 0;
	for(rowCounter = 2; rowCounter < table.rpws.length; rowCounter++)
	{
		homeScore = 0;
		oppScore = 0;
		score = table.rows[rowCounter].cells[4].innerHTML - table.rows[rowCounter].cells[3].innerHTML;

		if(score<0)
		{
			table.rows[rowCounter].cells[4].innerHTML = 'Opp';
		}
		else if(score>0)
		{
			table.rows[rowCounter].cells[4].innerHTML = 'Home';
		}
		else {
			{
				table.rows[rowCounter].cells[4].innerHTML = 'Tie';
			}
		}
